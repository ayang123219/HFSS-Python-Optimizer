"""HFSS AI代理优化器 - 主动学习框架
基于主动学习和混合代理模型（BNN）的HFSS天线参数优化系统
...
​    5.​模型重训练​​：用扩充后的数据集更新BNN模型。
​        ​终止条件​​：
            达到最大轮次（如20轮），或BNN的MSE收敛（如变化率<5%）。
​​四. 最优解提取（独立于主动学习）​​
​    1.​预筛选​​：
        -生成10,000组随机参数，用BNN预测特征向量。
        -基于约束条件（寻找损失值最低）过滤候选解，保留Top-K（如50组）。
    2.​HFSS验证​​：
        -将候选解送入HFSS仿真，剔除预测误差大的样本。
        -输出所有满足约束的解。
    3.​失败处理​​：
        -若验证后无解，将失败样本加入数据集，重新训练BNN并重复预筛选。
"""
...
from GPRtrainer import BNNTrainer, GPRTrainer
import psutil  # 添加这一行
...
class HfssAIAgentOptimizer:
    ...
    def __init__(self, 
                 ...
                 retry_delay: int = 10,  # 新增: 重试间隔(秒)
                 simulation_timeout: int = 1800,  # 新增: 仿真超时时间(秒)
                 ):
        ...
        self.model = None
        self.surrogate_trainer = None  # 新增：BNN训练器实例引用
        self.X_scaler = StandardScaler()
        self.y_scaler = StandardScaler()
        
        self.loss_history = []        # 存储每轮评估样本的最小损失
        self.mse_history = []         # 存储每轮BNN训练后的训练集MSE
    ...
    def optimize(self):
        ...
        try:
            ...
            if not self.train_feature_model():
                print("❌ BNN模型训练失败，终止优化")
            else:
                # +++ 新增：训练后立即验证模型 +++
                self.validate_model(n_samples=5)
                # 直接对比特征向量空间
                print("=== 特征向量空间验证 ===")
                self.validate_feature_space(n_samples=5)
                
                # 复数空间验证
                print("\n=== 复数空间验证 ===")
                complex_errors = self.validate_complex_space(n_samples=5)
                
                # 显示验证结果
                self.show_validation_results(complex_errors, n_samples=5)
    ...
    def extract_optimal_solutions(self, n_candidates=10000, top_k=3):
        ...
        print("🧠 使用BNN预测特征...")
        y_pred, _ = self.surrogate_trainer.predict(candidates)
    ...
    def train_feature_model(self) -> bool:
        ...
        try:
            ...
            trainer = BNNTrainer(
                dataset_path=os.path.join(self.save_dir, "dataset.npz"),
                output_dir=os.path.join(self.save_dir, "bnn_models"),
                n_units=128,
                n_samples=10,
                learning_rate=0.001
            )
            self.surrogate_trainer = trainer  # 使用通用名称
    ...
    def save_training_history(self):
        ...
        # 获取模型信息并转换为Python原生类型
        model_info = self.surrogate_trainer.get_model_summary() if self.surrogate_trainer else {}
        processed_info = {}
        
        for key, value in model_info.items():
            # 转换NumPy和TensorFlow类型为Python原生类型
            if isinstance(value, (np.floating, np.integer)):
                ...
                processed_info[key] = value.numpy().item()
            else:
        ...
        history = {
            ...
            **processed_info,  # 使用转换后的信息
            "active_cycle": self.active_cycle,
        }
        
        # 确保所有数值都是原生类型
        for key in list(history.keys()):
            if isinstance(history[key], np.generic):
    ...
    def active_learning_cycle(self):
        ...
        try:
            ...
            for cycle in range(1, self.max_active_cycles + 1):
                ...
                # 2. BNN预测与不确定性评估
                print("🧠 进行BNN预测...")
                y_pred, y_std = self.surrogate_trainer.predict(candidate_samples)
                ...
                # 在训练后记录MSE - 修复访问方式
                mse = self.surrogate_trainer.get_train_mse()  # 现在可以正确访问
    ...
    def evaluate_samples(self, samples: np.ndarray, eval_type: str):
        ...
        for i in range(samples.shape[0]):
            ...
            # 添加重试机制
            loss, success = self.run_sample_evaluation_with_retry(sample)
    ...
    def run_sample_evaluation_with_retry(self, sample: np.ndarray):
        """带重试机制的样本评估"""
        retry_count = 0
        loss = float('inf')
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                # 检查并确保HFSS连接有效
                if not self.is_hfss_alive():
                    print("⚠️ HFSS连接异常，尝试重启")
                    self.reconnect_hfss()
    ...
    def validate_feature_space(self, n_samples=5):
        """特征向量直接对比"""
        # 随机选择样本
        indices = np.random.choice(len(self.dataset.X), n_samples, replace=False)
        
        for i in indices:
            # 获取真实特征
            true_vec = np.array(self.dataset.feature_vectors[i])
            
            # 模型预测
            X = np.array([self.dataset.X[i]])
            pred_vec, _ = self.surrogate_trainer.predict(X)  # 使用训练器的预测方法
            
            # 计算误差
            mse = np.mean((true_vec - pred_vec[0])**2)
            mae = np.mean(np.abs(true_vec - pred_vec[0]))
            
            print(f"样本 {i+1}: MSE={mse:.6f}, MAE={mae:.6f}")
            
            # 可视化对比
            plt.figure(figsize=(10, 4))
            plt.plot(true_vec, 'b-', label='真实特征')
            plt.plot(pred_vec[0], 'r--', label='预测特征')
            plt.title(f"样本 {i+1} 特征向量对比 (MSE={mse:.4f})")
            plt.legend()
            plt.show()

    def plot_complex_comparison(self, result):
        """可视化复数空间对比"""
        for port_name in result['true']:
            plt.figure(figsize=(12, 8))
            
            # 幅度对比
            plt.subplot(2, 1, 1)
            true_mag = np.abs(result['true'][port_name])
            pred_mag = np.abs(result['pred'][port_name])
            plt.plot(true_mag, 'b-', label='真实幅度')
            plt.plot(pred_mag, 'r--', label='预测幅度')
            plt.title(f"{port_name} 幅度对比")
            plt.legend()
            
            # 相位对比
            plt.subplot(2, 1, 2)
            true_phase = np.angle(result['true'][port_name])
            pred_phase = np.angle(result['pred'][port_name])
            plt.plot(true_phase, 'b-', label='真实相位')
            plt.plot(pred_phase, 'r--', label='预测相位')
            plt.title(f"{port_name} 相位对比")
            plt.legend()
    ...
    def validate_model(self, n_samples=10):
        ...
        for idx in sample_indices:
            ...
            # 模型预测
            pred_features, _ = self.surrogate_trainer.predict(sample_params)
...
#===================================重启机制====================================
    def run_simulation_with_timeout(self) -> bool:
        """带超时监控的运行仿真"""
        if not self.hfss:
            raise RuntimeError("未连接到HFSS")
        
        print(f"🚀 启动仿真: {self.setup_name} (超时: {self.simulation_timeout}秒)...")
        start_time = time.time()
        ...
        # 等待线程完成或超时
        sim_thread.join(self.simulation_timeout)
        
        if sim_thread.is_alive():
            print(f"⏰ 仿真超时! 已超过 {self.simulation_timeout} 秒")
            raise TimeoutError(f"仿真超时 ({self.simulation_timeout}秒)")
        
        if sim_thread.exception:
            raise sim_thread.exception
        
        elapsed = time.time() - start_time
        print(f"✅ 仿真完成! 耗时: {elapsed:.2f}秒")
        return sim_thread.result
    ...
    def is_hfss_alive(self) -> bool:
        """检查HFSS连接是否有效"""
        try:
            if self.hfss and self.hfss.hfss:
                # 简单检查：尝试获取项目名称
                project_name = self.hfss.hfss.project_name
                return True
            return False
        except Exception:
            return False
    
    def kill_ansys_processes(self):
        """强制终止所有ANSYS相关进程"""
        print("🛑 强制终止ANSYS进程...")
        killed = False
        ...
        for proc in psutil.process_iter(['pid', 'name']):
            try:
                ...
                if "ansysedt" in process_name or "ansysedt" in process_name:
...
def main():
    ...
    VARIABLES = [
        ...
        {'name': 'Lp_extra', 'bounds': (1, 20), 'unit': 'mm'},
    ...
    optimizer = HfssAIAgentOptimizer(
        ...
        max_active_cycles=30,
        init_sample_multiplier=30,
        ei_balance=0.6,
        feature_freq_points=70,
        initial_dataset_path=r"C:\Users\Administrator\Desktop\huaSheng\python_HFSS\optim_results\ai_optim_20250729-182300\dataset.npz",
    )
